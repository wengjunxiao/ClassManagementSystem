...
namespace ClassManagementSystem.StudAtdInfoManage
{
    public partial class StudAtdInfoUC : UserControl
    {
        ...
        public StudAtdInfoUC(mainForm mf)
        {
            InitializeComponent();
        ...
        private void DoModify()
        {
            if (this.AttendancedataGridView.SelectedRows.Count > 0)
            {
                ...
                string timeStr = this.AttendancedataGridView.SelectedRows[0].Cells["考勤日期"].Value.ToString();
                if (timeStr == "")
                {
                    MessageBox.Show("请输入考勤日期！");
                    return;
                }
                DateTime atdtime = Convert.ToDateTime(timeStr);
                if (!IsItemExist(item))
                {
                    if (!this.AddItem(item))
                    {
        ...
        {
            string sqlcmd = string.Format("delete from Students_Attendance where Sno = '{0}'", sno);
        ...
        private void InitControl()
        {
            ...
            {
                this.InsertAtdBtn.Enabled = true;
